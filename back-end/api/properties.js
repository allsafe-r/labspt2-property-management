...
// Get all properties
router.get('/', (req, res) => {
	db.getProperties().then((properties) => res.status(200).json(properties)).catch((err) => {
...
router.get('/:houseId', (req, res) => {
	...
	db
		.findByPropertyId(houseId)
		.then((properties) => {
			if (properties) {
				res.status(200).json(properties);
			} else {
				res.status(404).json({ error: 'The specified property does not exist.' });
			}
		})
		.catch((err) => {
			res.status(500).json({ error: `${err}` });
	...
	db
		...
		.then((property) => {
			if (property) {
				...
				res.status(404).json({ error: 'The property specified does not exist.' });
			}
		})
		.catch((err) => {
			res.status(500).json({ error: `${err}` });
